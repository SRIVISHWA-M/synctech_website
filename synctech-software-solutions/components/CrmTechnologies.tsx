...
const categories = [
  ...
  },
  {
    ...
    pattern: 'radial-gradient(circle at 0% 100%, rgba(147, 51, 234, 0.05) 0%, transparent 50%)',
    tech: [
      { name: 'Power Automate', desc: 'Workflow automation across Microsoft services.', logo: 'https://res.cloudinary.com/dzf7f5ag3/image/upload/v1782132487/unnamed_dz1azo.webp' },
      { name: 'Zapier', desc: 'Connects apps and automates workflows.', logo: 'https://cdn.simpleicons.org/zapier/FF4A00' },
      { name: 'Make', desc: 'Visual automation platform for business processes.', logo: 'https://cdn.simpleicons.org/make/000000' },
      { name: 'HubSpot Workflows', desc: 'Automated customer engagement and nurturing.', logo: 'https://cdn.simpleicons.org/hubspot/FF7A59' },
    ]
  },
  {
    id: 'analytics',
...
const getCarouselPosition = (index: number, activeIndex: number) => {
  ...
  if (diff === 0) return { x: 0, scale: 1.15, opacity: 1, zIndex: 30, filter: 'blur(0px)' };
  if (diff === 1) return { x: '120%', scale: 0.85, opacity: 0.5, zIndex: 20, filter: 'blur(2px)' };
  if (diff === 2) return { x: '230%', scale: 0.7, opacity: 0.1, zIndex: 10, filter: 'blur(4px)' };
  if (diff === -1) return { x: '-120%', scale: 0.85, opacity: 0.5, zIndex: 20, filter: 'blur(2px)' };
  if (diff === -2) return { x: '-230%', scale: 0.7, opacity: 0.1, zIndex: 10, filter: 'blur(4px)' };
              ...
              >
                {categories.map((category) => (
                  ...
                  >
                    ...
                    <div className={`absolute inset-0 ${category.bg} opacity-10 pointer-events-none`} />

                    <div className="relative z-10">
                      <h3 className="text-2xl font-bold text-slate-800 mb-2">{category.title}</h3>
                      <p className="text-slate-500 text-sm leading-relaxed mb-6 max-w-xs">{category.description}</p>
                ...
                <div className="relative w-full h-[450px] flex items-center justify-center perspective-1000 mt-12">
                   {activeCategoryData.tech.map((tech, idx) => {
                     ...
                     return (
                       ...
                       >
                         {/* Platform Logo */}
                         <div className={`w-24 h-24 mb-8 flex items-center justify-center transition-transform duration-700 ${isCenter ? 'scale-110 drop-shadow-xl' : 'scale-90 grayscale opacity-70'}`}>
                         ...
                         {/* Platform Name */}
                         <h4 className={`text-2xl font-bold mb-4 transition-colors duration-500 ${isCenter ? 'text-slate-800' : 'text-slate-500'}`}>
                           {tech.name}
                         </h4>
